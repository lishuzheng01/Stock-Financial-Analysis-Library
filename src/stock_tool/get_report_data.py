...
def get_report_data(stock="", symbol="", transpose=True):
    '''
    获取股票的财务报告数据
    stock: 股票代码
    symbol: 报告类型，可选"资产负债表", "利润表", "现金流量表"
    transpose: 是否转置数据，默认为None，即根据数据自动判断
    demo:df = get_report_data(stock="600519", symbol="利润表", transpose=True, )
    '''
# 财报列名中英文映射字典
    translation_map_income = {# --- 基础信息 ---
    "报告日": "Report Date",
...
# 从新浪财经获取财报数据，添加SSL错误处理
    import requests
    import urllib3
    
    # 禁用SSL警告和验证以处理SSL连接问题
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
    
    try:
        # 尝试使用SSL验证关闭的方式获取数据
        df_sina = ak.stock_financial_report_sina(stock=stock, symbol=symbol)
    except Exception as e:
        # 如果SSL连接失败，尝试使用备用方法
        print(f"警告: 从Sina获取财务数据失败: {e}")
        print("尝试使用备用数据源...")
        ...
        # 记录错误信息
        print(f"无法获取股票 {stock} 的 {symbol} 数据，请检查网络连接或稍后重试")
    # transpose 默认为True
    if symbol == "资产负债表":
        if transpose == None:
            df_sina =  df_sina.rename(columns=translation_map_asset)
        ...
        if transpose == None:
            df_sina =  df_sina.rename(columns=translation_map_income)
        elif transpose == False:
            df_sina = df_sina
        elif transpose == True:
            df_sina = df_sina.rename(columns=translation_map_income)
        ...
        if transpose == None:
            df_sina =  df_sina.rename(columns=translation_map_cashflow)
        elif transpose == False:
            df_sina = df_sina
        elif transpose == True:
            df_sina = df_sina.rename(columns=translation_map_cashflow)
    ...
    return df_sina
# symbol 参数可选: "资产负债表", "利润表", "现金流量表"
# df_sina 类型是 pandas.DataFrame
